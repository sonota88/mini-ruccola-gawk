...
run_test_json() {
  ...
  cat $infile | $RUNNER_CMD test/test_json.awk
}

run_lex() {
  local infile="$1"; shift

  cat $infile | $RUNNER_CMD mrcl_lexer.awk
}

run_parse() {
  local infile="$1"; shift

  cat $infile | $RUNNER_CMD mrcl_parser.awk
}

run_codegen() {
  local infile="$1"; shift

  cat $infile | $RUNNER_CMD mrcl_codegen.awk
}
...
test_compile_nn() {
  ...
  cp $temp_tokens_file ${temp_tokens_file}_bu # TODO
  ...
  cp $temp_vgt_file ${temp_vgt_file}_bu # TODO
